...
from setuptools.build_meta import build_sdist as _build_sdist, build_wheel as _build_wheel
...
def get_root():
    ...
    root = os.path.realpath(os.path.abspath(Path(__file__).parent.parent))
    print(root)
    setup_py = os.path.join(root, "setup.py")
    versioneer_py = os.path.join(root, "build_backend", "build_backend.py")
...
def get_config_from_root(root):
    ...
    def get(cp, name):
        if cp.has_option("versioneer", name):
            return cp.get("versioneer", name)
        return None
...
def do_vcs_install(manifest_in, versionfile_source, ipy):
    ...
    try:
        ...
        versioneer_file = "build_backend.py"
...
cmds["build_ext"] = BuildExtCommand
...
class BuildExeCommand(_build_exe):
    def run(self):
        root = get_root()
        cfg = get_config_from_root(root)
        versions = get_versions()
...
cmds["build_exe"] = BuildExeCommand
# del cmds["build_py"]
...
class SdistCommand(_sdist):
    def __init__(self, *args, **kwargs):
        super(SdistCommand, self).__init__(*args, **kwargs)
